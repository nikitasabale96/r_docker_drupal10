...
class UploadExamplesEditForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, int $example_id = NULL) {
    ...
    if ($example_data->approval_status != 0) {
      $this->messenger()->addError($this->t('You cannot edit an example after it has been approved or dis-approved. Please contact site administrator if you want to edit this example.'));
      $form_state->setRedirect('<front>');
      return [];
    }
    /* get examples files */
    ...
    if ($preference_data->approval_status != 1) {
      $this->messenger()->addError($this->t('Cannot edit example. Either the book proposal has not been approved or it has been rejected.'));
      $form_state->setRedirect('<front>');
      return [];
    }
  ...
  public function submitForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    $param['example_updated']['user_id'] = $user_id;
    ...
    $result = \Drupal::service('plugin.manager.mail')->mail('textbook_companion', 'example_updated', $email_to, $langcode, $param, $from, TRUE);
